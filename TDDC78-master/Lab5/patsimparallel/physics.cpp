...
float wall_collide(Particle& p, Wall& wall){
    float gPreassure = 0.0 ;

    if (wall.leftsolid && p.x < wall.x0) {
    	p.vx = -p.vx ;
    	p.x  = wall.x0 + (wall.x0-p.x);
    	gPreassure += 2.0*fabs(p.vx) ;
    }
    if (wall.rightsolid && p.x > wall.x1) {
    	p.vx = -p.vx ;
    	p.x  = wall.x1 - (p.x-wall.x1);
    	gPreassure += 2.0*fabs(p.vx) ;
    }
    if (wall.upsolid && p.y < wall.y0) {
    	p.vy = -p.vy ;
    	p.y  = wall.y0 + (wall.y0-p.y);
    	gPreassure += 2.0*fabs(p.vy) ;
    }
    if (wall.downsolid && p.y > wall.y1) {
    	p.vy = -p.vy ;
...
float collide(Particle& p1, Particle& p2){
    ...
    a=std::pow(p1.vx-p2.vx,2)+std::pow(p1.vy-p2.vy,2);
    b=2*((p1.x - p2.x)*(p1.vx - p2.vx)+(p1.y - p2.y)*(p1.vy - p2.vy));
    c=std::pow(p1.x-p2.x,2)+std::pow(p1.y-p2.y,2)-4*1*1;

    if (a!=0.0){
    	temp=std::pow(b,2)-4*a*c;
    	if (temp>=0){
    	    temp=std::sqrt(temp);
    	    t1=(-b+temp)/(2*a);
    	    t2=(-b-temp)/(2*a);
...
void interact(Particle& p1, Particle& p2, float t){
    ...
    if (t>=0) {
    	...
    	if (p2.y==0){
    	    ...
    	    if (fabs(b)>fabs(a)){
        		tao=-a/b;
        		s=1/(std::sqrt(1+std::pow(tao,2)));
        		c=s*tao;
    	    }
    	    else{
        		tao=-b/a;
        		c=1/(std::sqrt(1+std::pow(tao,2)));
        		s=c*tao;
    	    }
    	}
