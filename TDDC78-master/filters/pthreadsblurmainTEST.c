...
void *myThreadFun(void *args){
	...
	pblurfilter(xsize,yrows, src, radius, w, displs/xsize);

}
void *myThreadFunTranspose(void *args){
	
	arg_struct *data = (arg_struct *)args;

	pixel *src = data->src;
	pixel *transpose = data->transpose;
	...
	int j,i;
	int startindex=	displs/xsize;
	printf("startindex: %d , xsize: %d, yrows: %d \n",startindex,xsize,yrows);
	for(i = startindex; i<startindex+yrows;i++){
		for(j=0;j<xsize;j++){
			transpose[j*ysize + i]=src[j + i*xsize];		
		}
	}
}
...
int main (int argc, char ** argv) {
	...
	for(i=0;i<p;i++){
		data[i].src=src;
		data[i].transpose=transpose;
		
		pthread_create(&tid[i], NULL, myThreadFunTranspose, (void *)&data[i]);
	}
	...
	for(i=0;i<p;i++){
		...
		data[i].xsize = ysize;
		data[i].ysize = xsize;
		data[i].yrows = sendcount[i]/ysize;
		data[i].displs = displs[i];
